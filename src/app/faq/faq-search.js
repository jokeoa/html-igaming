$(document).ready(function() {
  ...
  $('.accordion-item').each(function() {
    ...
    const dataParent = $collapse.attr('data-bs-parent') || null;
    const question = $button.text().trim();
    const answer = $item.find('.accordion-body').text().trim();
    faqData.push({
      question: question,
      answer: answer,
      element: $item,
  ...
  function performSearch(searchTerm) {
    ...
    if (searchLower === '') {
      // Restore accordion single-open behavior and collapse all
      faqData.forEach(function(faq) {
        if (faq.dataParent) {
          faq.collapseEl.attr('data-bs-parent', faq.dataParent);
        }
        faq.element.show().removeClass('search-match');
        if (faq.collapseEl && faq.collapseEl.length && typeof bootstrap !== 'undefined' && bootstrap.Collapse) {
          const collapseNode = faq.collapseEl[0];
          const instance = bootstrap.Collapse.getOrCreateInstance(collapseNode, { toggle: false });
          instance.hide();
        }
      });
    ...
    // Allow multiple items to be open during search
    faqData.forEach(function(faq) {
      faq.collapseEl.removeAttr('data-bs-parent');
    });

    // Task 1: Filter items and expand all matches
    let matchCount = 0;
    faqData.forEach(function(faq) {
      ...
      if (questionMatch || answerMatch) {
        ...
        matchCount++;
        // Expand all matched accordion items
        if (faq.collapseEl && faq.collapseEl.length && typeof bootstrap !== 'undefined' && bootstrap.Collapse) {
          const collapseNode = faq.collapseEl[0];
          const instance = bootstrap.Collapse.getOrCreateInstance(collapseNode, { toggle: false });
          instance.show();
        ...
        // Ensure non-matching items are collapsed
        if (faq.collapseEl && faq.collapseEl.length && typeof bootstrap !== 'undefined' && bootstrap.Collapse) {
          const collapseNode = faq.collapseEl[0];
          const instance = bootstrap.Collapse.getOrCreateInstance(collapseNode, { toggle: false });
          instance.hide();
